...
The `training/` directory contains everything needed to train and evaluate a basic model based on the metrics collected by the monitoring script. 
- `training/clean_dataset.py` takes a `.parquet` file as argument and aggregates values for the same timestamp together. Also only a selected subset of features is retained, these can be defined in the script itself. (In the future the `--feature` flag will be useable).  
Usage: `python3 clean_dataset.py -f data/raw_datasets/process_interval_data-3.parquet`

- `training/ridge-regression-training.py` takes a `.parquet` file generated by `training/clean_dataset.py` and trains a L2 Ridge Regression on the dataset. The finished model and scaler used to normalize input are saved to `models/`.  
Usage: `python3 ridge-regression-training.py --dataSource data/raw_datasets/process_interval_data-3-cleaned.parquet`

- `training/ridge-regression-prediction.py` takes a `.parquet` file generated by `training/clean_dataset.py` as well as the model and scaler generated by the `training/ridge-regression-training.py` script to make a prediction on the data. A sample of the result is plotted and saved to `training/plots/`  
Usage: `python3 ridge-regression-prediction.py --scaler models/l2-regression-0616221414-scaler.npy --modelFile models/l2-regression-0616221414 --dataSource data/raw_datasets/process_interval_data-2-cleaned.parquet`
...
- `training/` — various scripts related to training and evaluating   
